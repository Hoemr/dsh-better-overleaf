/**
 * Direct-CDP Overleaf login. Launches a user-selected Chromium-family browser
 * with a dedicated (persistent by default) profile and a freshly reserved
 * loopback CDP port, waits for the user to log in, then reads Overleaf cookies
 * with the browser-level `Network.getAllCookies` command. No Playwright browser
...
import { OVERLEAF_COOKIE } from './credentials.ts'
import type { OverleafLoginResult } from './types.ts'

/** Browser choices exposed to the client. */
export type OverleafBrowserChannel = 'auto' | 'default' | 'msedge' | 'chrome' | 'real'

/**
 * How the login browser profile is managed. `persistent` (the default) keeps a
 * dedicated dsh profile on disk so the Overleaf session — and any OAuth-linked
 * browser account — survives across logins: credentials are entered once, and
 * later logins capture the cookie without typing anything. `temporary` uses a
 * throwaway profile wiped after every attempt.
 */
...
export interface LoginOptions {
  ...
  /** Explicit Chromium-family executable; tried first when set (third-party browsers). */
  browserPath?: string
  headless: boolean
  timeoutMs: number
  /** Profile persistence mode; defaults to persistent. */
...
/** One CDP cookie record from `Network.getAllCookies`. */
interface CdpCookie {
  name: string
  value: string
  domain: string
...
/** Minimal promise-based CDP client over the browser WebSocket endpoint. */
class CdpClient {
  private nextId = 1
  ...
  /** Open the browser-level WebSocket debugger endpoint. */
  static async connect(url: string, timeoutMs = 5_000): Promise<CdpClient> {
    const ws = new WebSocket(url)
    await new Promise<void>((resolve, reject) => {
      const timer = setTimeout(() => {
          ws.close()
          reject(new Error(`overleaf: CDP WebSocket timed out for ${url}`))
        }, timeoutMs)
        ws.onopen = () => { clearTimeout(timer); resolve() }
      ws.onerror = () => { clearTimeout(timer); reject(new Error(`overleaf: CDP WebSocket failed for ${url}`)) }
    })
  ...
  call(method: string, params: Record<string, unknown> = {}, timeoutMs = 5_000): Promise<unknown> {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pending.delete(id)
        reject(new Error(`overleaf: CDP ${method} timed out`))
      }, timeoutMs)
      this.pending.set(id, {
...
/** Locate the Windows default-browser executable through the shell association. */
...
function candidatesFor(channel: OverleafBrowserChannel, browserPath?: string): BrowserCandidate[] {
  ...
  if (channel === 'real') {
    // The user's real profile: their own default/known browser, launched
    // WITHOUT a user-data-dir override so saved accounts and passwords apply.
...
/**
 * Reserve one loopback TCP port for the browser CDP endpoint. The port is
 * released immediately and re-bound by Chromium when it starts; polling the
 * explicit port is more reliable with third-party Chromium builds than reading
 * a `DevToolsActivePort` file written by `--remote-debugging-port=0`.
 */
async function findFreeCdpPort(): Promise<number> {
  ...
  if (port === undefined || !Number.isInteger(port) || port <= 0) {
    throw new Error('overleaf: could not reserve a local CDP port')
  }
  return port
}
...
function browserProcessFailure(child: ChildProcess): Promise<never> {
  return new Promise<never>((_, reject) => {
    ...
    child.once('exit', (code, signal) => {
      reject(new Error(`overleaf: browser exited before CDP was ready (code=${String(code)}, signal=${String(signal)})`))
    })
  })
}
...
async function connectCdpWithRetry(port: number, timeoutMs: number): Promise<CdpClient> {
  ...
  throw new Error(`overleaf: browser did not expose CDP on 127.0.0.1:${port} within ${timeoutMs}ms`)
}
...
async function connectCdp(port: number): Promise<CdpClient> {
  ...
  if (!response.ok) throw new Error(`overleaf: CDP endpoint returned HTTP ${response.status} on 127.0.0.1:${port}`)
  const version = await response.json() as { webSocketDebuggerUrl?: string }
  if (version.webSocketDebuggerUrl === undefined) throw new Error('overleaf: CDP endpoint has no webSocketDebuggerUrl')
  return await CdpClient.connect(version.webSocketDebuggerUrl)
}

/** Read all cookies from one CDP connection, trying browser-level cookie APIs. */
  ...
  for (const method of ['Storage.getCookies', 'Network.getAllCookies'] as const) {
    try {
      ...
      if (!Array.isArray(result.cookies)) {
        throw new Error(`overleaf: CDP ${method} returned no cookie array`)
      }
...
async function connectFirstPageCdp(port: number, timeoutMs: number): Promise<CdpClient> {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (!response.ok) throw new Error(`overleaf: CDP target list returned HTTP ${response.status}`)
      const targets = await response.json() as Array<{ type?: string; webSocketDebuggerUrl?: string }>
      const page = targets.find(target => target.type === 'page')
      const pageUrl = page?.webSocketDebuggerUrl
      if (pageUrl === undefined) throw new Error('overleaf: CDP target list has no page WebSocket')
      return await CdpClient.connect(pageUrl)
    } catch {
      // The login tab's page target appears right after startup; keep polling briefly.
    }
    await sleep(300)
  }
  throw new Error(`overleaf: browser exposed no page CDP target on 127.0.0.1:${port} within ${timeoutMs}ms`)
}
...
/** Read Overleaf cookies through CDP once the user has logged in. */
  ...
  try {
    while (Date.now() < deadline) {
      try {
        ...
        const overleafCookies = cookies.filter(cookie =>
          cookie.domain === 'overleaf.com' || cookie.domain.endsWith('.overleaf.com'))
        const header = overleafCookies.map(cookie => `${cookie.name}=${cookie.value}`).join('; ')
        const authenticated = overleafCookies.some(cookie =>
          /^(overleaf_session|sharelatex_session)/i.test(cookie.name) && cookie.value !== '')
        if (authenticated && header !== '') {
          let reachedProject = false
          try {
            reachedProject = (await pageTargetUrls(cdpPort)).some(url => url.startsWith(projectUrlPrefix))
          } catch {
    ...
    throw new Error('overleaf: did not detect a logged-in Overleaf session before timeout')
  } finally {
    pageCdp?.close()
  }
}
...
async function loginWithExecutable(executablePath: string, options: LoginOptions): Promise<string> {
  ...
  if (realProfile) {
    // Real profile mode: no --user-data-dir override, so saved accounts and
    // passwords apply. The browser keeps running after capture.
  ...
  try {
    ...
    return await captureCookies(cdp, cdpPort, options.projectUrlPrefix, options.timeoutMs)
  } catch (error) {
    if (realProfile) {
      throw new Error(
...
function openDefaultBrowser(url: string): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once('close', code => {
      if (code === 0) resolve()
      else reject(new Error(`overleaf: default-browser launcher exited ${String(code)}`))
    })
  })
}

/**
 * Run the browser login flow. Returns automatic when cookies were captured,
...
export async function loginWithPlaywright(
  ...
  for (const candidate of candidatesFor(options.browserChannel, options.browserPath)) {
    try {
      ...
      await credentials.set(OVERLEAF_COOKIE, cookie)
      return { kind: 'automatic' }
    } catch (error) {
      failures.push(`${candidate.label}: ${error instanceof Error ? error.message : String(error)}`)
    }
  }
  await openDefaultBrowser(options.loginUrl)
  return {
    kind: 'manual',
    loginUrl: options.loginUrl,
    instructions: failures.length === 0
      ? 'Log in to Overleaf in your default browser, then copy the overleaf_session2 cookie (DevTools > Application > Cookies > https://www.overleaf.com); document.cookie cannot read it because it is httpOnly.'
      : `Automatic cookie capture failed for: ${failures.join(' | ')}. Log in in the opened browser, then copy the httpOnly overleaf_session2 cookie from DevTools > Application > Cookies.`,
  }
}
